...
const App = () => {
  ...
  // ... (Rest of the component logic: Log watchers, Rally Helpers, etc. remain the same)
  // ... (Paste the rest of the component logic from previous version here, ensuring no duplicates)
  ...
  // RESET BATTLE FUNCTION (Updated to clear Firestore/State properly)
  const resetBattle = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 md:p-8 flex flex-col items-center">
      <div className="max-w-4xl w-full space-y-6">
        ...
        {/* ... (Calculator, Rally Manager, Grouping tabs content essentially unchanged) ... */}
        {currentTab === 'calculator' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-left-4 duration-300">
                ...
                <div className="bg-black rounded-xl border border-slate-700 overflow-hidden relative group aspect-video flex items-center justify-center">
                  {!isScreenShared && (
                    <div className="flex flex-col items-center justify-center p-12 text-center">
                      ...
                      <button onClick={startScreenShare} className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-lg font-bold border border-slate-600 transition-all"><Eye size={16} /> Select Game Screen</button>
                    </div>
                  )}
                  <video ref={videoRef} autoPlay onClick={handleVideoClick} className={`w-full h-full object-contain cursor-crosshair ${isScreenShared ? 'block' : 'hidden'}`} />
                  <canvas ref={canvasRef} className="hidden" />
                  {isScreenShared && targetPixel && (
                    <div className="absolute pointer-events-none transform -translate-x-1/2 -translate-y-1/2 border-2 border-green-400 shadow-[0_0_10px_rgba(74,222,128,0.5)] z-10" style={{ left: `${targetPixel.x * 100}%`, top: `${targetPixel.y * 100}%`, width: `${scanSize}px`, height: `${scanSize}px` }}>
                        <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-black/70 text-[10px] text-green-400 px-1 rounded whitespace-nowrap">SCAN AREA</div>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-700">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                      <button onClick={() => { setCastleOwner('us'); setAutoTrackEnabled(false); }} className={`px-3 py-2 rounded-lg font-bold text-sm transition-all border ${castleOwner === 'us' ? 'bg-blue-600 border-blue-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>WE OCCUPY</button>
                      <button onClick={() => { setCastleOwner('neutral'); setAutoTrackEnabled(false); }} className={`px-3 py-2 rounded-lg font-bold text-sm transition-all border ${castleOwner === 'neutral' ? 'bg-amber-600 border-amber-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>NOBODY</button>
                      <button onClick={() => { setCastleOwner('enemy'); setAutoTrackEnabled(false); }} className={`px-3 py-2 rounded-lg font-bold text-sm transition-all border ${castleOwner === 'enemy' ? 'bg-red-600 border-red-400 text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>ENEMY OCCUPIES</button>
                  </div>
                </div>

                <div className="bg-slate-900 p-4 rounded-xl border border-slate-700">
                  <div className="flex items-center gap-2 mb-3 border-b border-white/10 pb-2"><Settings size={16} className="text-indigo-400"/><span className="text-xs font-bold uppercase text-slate-300">Auto-Tracker Setup</span></div>
                  <div className="space-y-3 text-sm">
                    <div className="bg-black/20 p-2 rounded border border-white/5 mb-2">
                        <div className="flex justify-between items-center text-xs text-slate-400 mb-1"><span className="flex items-center gap-1"><Maximize size={12}/> Scan Box Size</span><span className="font-mono text-indigo-300">{scanSize}px</span></div>
                        <input type="range" min="10" max="150" value={scanSize} onChange={(e) => setScanSize(parseInt(e.target.value))} className="w-full h-1 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                    </div>
                    ...
                    <button onClick={() => capturePattern('us')} disabled={!targetPixel} className="w-full flex justify-between items-center px-3 py-2 bg-blue-900/20 border border-blue-500/30 rounded hover:bg-blue-900/40 disabled:opacity-50"><span className="text-blue-300 font-bold">Capture Area as OURS</span><div className="flex items-center gap-2">{debugDiff.us !== null && <span className="text-[10px] font-mono text-slate-400">Diff:{debugDiff.us}</span>}{hasUsPattern && <div className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_5px_cyan]"/>}</div></button>
                    <button onClick={() => capturePattern('enemy')} disabled={!targetPixel} className="w-full flex justify-between items-center px-3 py-2 bg-red-900/20 border border-red-500/30 rounded hover:bg-red-900/40 disabled:opacity-50"><span className="text-red-300 font-bold">Capture Area as ENEMY</span><div className="flex items-center gap-2">{debugDiff.enemy !== null && <span className="text-[10px] font-mono text-slate-400">Diff:{debugDiff.enemy}</span>}{hasEnemyPattern && <div className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_5px_orange]"/>}</div></button>
                    <div className="pt-2 border-t border-white/10 mt-2">
                      <div className="flex justify-between items-center text-xs text-slate-400 mb-1"><span>Pattern Match Tolerance</span><span className="font-mono text-indigo-300">{colorTolerance}</span></div>
                      <input type="range" min="5" max="100" value={colorTolerance} onChange={(e) => setColorTolerance(parseInt(e.target.value))} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer mb-3 accent-indigo-500" title="Increase if status stays on Neutral" />
                      <div className="text-center mb-2"><span className={`text-xs font-bold px-2 py-0.5 rounded ${debugMatch === 'us' ? 'bg-blue-900 text-blue-200' : debugMatch === 'enemy' ? 'bg-red-900 text-red-200' : 'bg-slate-800 text-slate-400'}`}>MATCH: {debugMatch === 'us' ? 'WE OCCUPY' : debugMatch === 'enemy' ? 'ENEMY' : 'NOBODY'}</span></div>
                      <button onClick={() => setAutoTrackEnabled(!autoTrackEnabled)} disabled={!hasUsPattern || !hasEnemyPattern} className={`w-full py-2 rounded font-bold text-center transition-all ${autoTrackEnabled ? 'bg-indigo-500 text-white animate-pulse' : 'bg-slate-700 text-slate-400'}`}>{autoTrackEnabled ? 'AUTO-TRACKING ON' : 'ENABLE AUTO-TRACK'}</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <TimeInput label={<><Shield size={16} className="text-blue-400"/> Our Occupation</>} value={ourTime} onChange={setOurTime} colorClass="bg-blue-500 border-blue-400" readOnly={true} />
              <TimeInput label={<><Swords size={16} className="text-red-400"/> Enemy Occupation</>} value={enemyTime} onChange={setEnemyTime} colorClass="bg-red-500 border-red-400" readOnly={true} />
            </div>
            <div className="w-full"><TimeInput label={<><Hourglass size={16} className="text-amber-400"/> Battle Time Remaining</>} value={remainingTime} onChange={setRemainingTime} colorClass="bg-amber-500 border-amber-400" readOnly={true} /></div>

            {results && (
              <div className={`rounded-2xl border-2 p-6 transition-all duration-300 shadow-2xl ${getStatusColor()}`}>
                <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4"><span className="text-lg font-bold uppercase tracking-widest opacity-80">Battle Status</span><div className="text-xl font-black uppercase flex items-center gap-2">{results.status === 'won' && <><CheckCircle className="text-green-400"/> Victory Secure</>}{results.status === 'lost' && <><AlertTriangle className="text-red-400"/> Defeat Imminent</>}{results.status === 'contested' && <><Swords className="text-yellow-400"/> Contested</>}</div></div>
                {battleLog.length > 0 && (
                   <div className="mb-6 p-4 border-b border-white/10 bg-slate-900/30 rounded-lg">
                       <VisualTimeline logs={[...battleLog, { id: Date.now(), timestamp: new Date().toISOString().replace('T', ' ').split('.')[0] + ' UTC', type: 'current', message: 'Now', side: 'neutral' }]} />
                   </div>
                )}
                {results.status === 'contested' && (
                  <div className="space-y-6">
                    <div className="bg-black/30 p-4 rounded-lg border border-white/5"><div className="flex justify-between items-start"><div><h3 className="text-blue-300 font-bold mb-1">Occupation Needed to Win</h3><p className="text-xs text-slate-400 max-w-[250px]">You must hold the castle for this total cumulative duration (starting now) to guarantee a win.</p></div><div className="text-right"><span className="text-3xl font-mono font-bold text-blue-400 block">{formatMinSec(results.secondsNeededToWin)}</span></div></div></div>
                    <div className="bg-red-950/30 p-4 rounded-lg border border-red-500/20"><div className="flex justify-between items-start"><div><h3 className="text-red-300 font-bold mb-1 flex items-center gap-2"><Clock size={16}/> Point of No Return</h3><p className="text-xs text-slate-400 max-w-[250px]">If the enemy holds the castle continuously from now, you will lose the mathematical possibility to win in:</p></div><div className="text-right"><span className="text-3xl font-mono font-bold text-red-400 block">{results.secondsUntilPointOfNoReturn > 0 ? formatMinSec(results.secondsUntilPointOfNoReturn) : "NOW"}</span></div></div></div>
                  </div>
                )}
                {results.status === 'won' && ( <div className="text-center py-4"><p className="text-green-200">Victory is mathematically guaranteed.</p></div> )}
                {results.status === 'lost' && ( <div className="text-center py-4"><p className="text-red-200">Victory is mathematically impossible.</p></div> )}
                <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-white/10">
                  <div className="text-center"><p className="text-xs text-slate-400 uppercase">Current Gap</p><p className={`font-mono text-lg font-bold ${results.leader === 'us' ? 'text-green-400' : 'text-red-400'}`}>{results.leader === 'us' ? '+' : '-'}{formatMinSec(results.gap)}</p></div>
                  <div className="text-center"><p className="text-xs text-slate-400 uppercase">Total Event Time</p><p className="font-mono text-lg font-bold text-slate-300">5h 00m 00s</p></div>
                </div>
              </div>
            )}
          </div>
        ...
        {currentTab === 'rallies' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
             <div className="flex justify-between items-center mb-4 bg-slate-900/50 p-3 rounded-lg border border-slate-700">
                <div className="flex items-center gap-2"><h2 className="text-lg font-bold text-slate-200">Rally Management</h2><button onClick={downloadTemplate} className="text-xs text-slate-500 hover:text-indigo-400 flex items-center gap-1 border border-slate-700 hover:border-indigo-500 px-2 py-1 rounded transition-colors" title="Download CSV Template"><FileDown size={14} /> Template</button></div>
                <div className="flex items-center gap-2"><input type="file" accept=".csv" ref={fileInputRef} onChange={handleFileChange} className="hidden" /><button onClick={handleImportClick} className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded text-xs font-bold flex items-center gap-2 transition-colors shadow-lg shadow-indigo-900/20"><Upload size={14} /> Import CSV</button></div>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="bg-black/20 rounded-xl p-4 border border-slate-800 flex flex-col gap-4">
                    <div className="flex items-center justify-between border-b border-blue-500/30 pb-3"><h2 className="text-xl font-bold text-blue-400 flex items-center gap-2"><Shield className="fill-blue-500/20"/> OUR RALLIES</h2><button onClick={() => addRallyLead('us')} className="bg-blue-600 hover:bg-blue-500 text-white p-1.5 rounded-md transition-colors"><Plus size={20} /></button></div>
                    <div className="flex flex-col gap-3">
                      {rallyLeads.filter(l => l.side === 'us').map(lead => ( <RallyLeadCard key={lead.id} lead={lead} onUpdate={updateRallyLead} onDelete={deleteRallyLead} /> ))}
                      {rallyLeads.filter(l => l.side === 'us').length === 0 && ( <div className="text-slate-600 text-center py-8 text-sm italic">No rally leads added.</div> )}
                    </div>
                </div>
                <div className="bg-black/20 rounded-xl p-4 border border-slate-800 flex flex-col gap-4">
                    <div className="flex items-center justify-between border-b border-red-500/30 pb-3"><h2 className="text-xl font-bold text-red-400 flex items-center gap-2"><Swords className="fill-red-500/20"/> ENEMY RALLIES</h2><button onClick={() => addRallyLead('enemy')} className="bg-red-600 hover:bg-red-500 text-white p-1.5 rounded-md transition-colors"><Plus size={20} /></button></div>
                    <div className="flex flex-col gap-3">
                      {rallyLeads.filter(l => l.side === 'enemy').map(lead => ( <RallyLeadCard key={lead.id} lead={lead} onUpdate={updateRallyLead} onDelete={deleteRallyLead} /> ))}
                      {rallyLeads.filter(l => l.side === 'enemy').length === 0 && ( <div className="text-slate-600 text-center py-8 text-sm italic">No enemy rally leads added.</div> )}
                    </div>
                </div>
             </div>
          </div>
        ...
        {currentTab === 'grouping' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-right-4 duration-300">
             <div className="bg-black/20 rounded-xl p-4 border border-slate-800 flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-blue-500/30 pb-3"><h2 className="text-xl font-bold text-blue-400 flex items-center gap-2"><Shield className="fill-blue-500/20"/> OUR GROUPS</h2><button onClick={() => addGroup('us')} className="bg-blue-600 hover:bg-blue-500 text-white p-1.5 rounded-md transition-colors" title="Add New Group"><Plus size={16} /></button></div>
                <div className="flex flex-col gap-4">
                   {groups.filter(g => g.side === 'us').map(group => ( <RallyGroupCard key={group.id} group={group} members={rallyLeads.filter(l => l.groupId === group.id)} availableLeads={rallyLeads.filter(l => l.side === 'us' && !l.groupId)} onAssign={assignLeadToGroup} onRemove={removeLeadFromGroup} onActivatePet={handleActivatePetFromGroup} onDelete={deleteGroup} onUpdate={updateRallyLead} /> ))}
                </div>
             </div>
             <div className="bg-black/20 rounded-xl p-4 border border-slate-800 flex flex-col gap-4">
                <div className="flex items-center justify-between border-b border-red-500/30 pb-3"><h2 className="text-xl font-bold text-red-400 flex items-center gap-2"><Swords className="fill-red-500/20"/> ENEMY GROUPS</h2><button onClick={() => addGroup('enemy')} className="bg-red-600 hover:bg-red-500 text-white p-1.5 rounded-md transition-colors" title="Add New Group"><Plus size={16} /></button></div>
                <div className="flex flex-col gap-4">
                   {groups.filter(g => g.side === 'enemy').map(group => ( <RallyGroupCard key={group.id} group={group} members={rallyLeads.filter(l => l.groupId === group.id)} availableLeads={rallyLeads.filter(l => l.side === 'enemy' && !l.groupId)} onAssign={assignLeadToGroup} onRemove={removeLeadFromGroup} onActivatePet={handleActivatePetFromGroup} onDelete={deleteGroup} onUpdate={updateRallyLead} /> ))}
                </div>
             </div>
          </div>
        )}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[calc(100vh-200px)]">
                     <div className="bg-slate-900/50 rounded-xl border border-slate-800 overflow-hidden flex flex-col md:col-span-1">
                        <div className="p-3 bg-slate-900 flex flex-col gap-2 border-b border-slate-800">
                           <div className="flex justify-between items-center font-bold text-slate-400 text-sm"><span>SAVED BATTLES</span><span className="text-xs bg-slate-800 px-2 py-0.5 rounded-full">{savedBattles.length}</span></div>
                           <div className="flex gap-2"><button onClick={handleHistoryImportClick} className="flex-1 bg-slate-800 hover:bg-indigo-900/50 text-slate-300 text-[10px] font-bold py-1.5 rounded border border-slate-700 hover:border-indigo-500 transition-all flex items-center justify-center gap-1"><Upload size={12} /> Import JSON</button><input type="file" accept=".json" ref={historyFileInputRef} onChange={handleHistoryFileChange} className="hidden" /></div>
                        </div>
                        <div className="overflow-y-auto flex-1 p-2 space-y-2 custom-scrollbar">
                           ...
                           {savedBattles.map(battle => (
                              <div key={battle.id} className={`p-3 rounded cursor-pointer group relative border transition-all ${selectedBattle && selectedBattle.id === battle.id ? 'bg-slate-700 border-indigo-500' : 'bg-slate-800 border-transparent hover:bg-slate-700 hover:border-slate-600'}`} onClick={() => setSelectedBattle(battle)}>
                                 <div className="flex justify-between items-start"><div><h4 className="font-bold text-slate-200 text-sm truncate max-w-[150px]">{battle.name}</h4><p className="text-[10px] text-slate-500 mt-1">{new Date(battle.date).toLocaleDateString()}</p></div>{battle.winner === 'us' ? <span className="text-[10px] font-bold text-green-400 bg-green-900/20 px-1.5 py-0.5 rounded border border-green-900/50">WIN</span> : <span className="text-[10px] font-bold text-red-400 bg-red-900/20 px-1.5 py-0.5 rounded border border-red-900/50">LOSS</span>}</div>
                                 <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={(e) => { e.stopPropagation(); exportBattle(battle); }} className="p-1.5 text-slate-400 hover:text-indigo-400 bg-slate-900 rounded border border-slate-700 hover:border-indigo-500" title="Download JSON"><Download size={12} /></button><button onClick={(e) => { e.stopPropagation(); deleteSavedBattle(battle.id); }} className="p-1.5 text-slate-400 hover:text-red-400 bg-slate-900 rounded border border-slate-700 hover:border-red-500" title="Delete"><Trash2 size={12} /></button></div>
                                 <div className="mt-3 pt-2 border-t border-white/5 grid grid-cols-2 gap-2 text-[10px]"><div className="text-center"><span className="text-slate-500 block">OUR TIME</span><span className="font-mono text-blue-400">{formatMinSec(toSeconds(battle.finalOurTime))}</span></div><div className="text-center"><span className="text-slate-500 block">ENEMY TIME</span><span className="font-mono text-red-400">{formatMinSec(toSeconds(battle.finalEnemyTime))}</span></div></div>
                              </div>
                           ))}
                        </div>
                     </div>
                     <div className="bg-black/20 rounded-xl border border-slate-800 md:col-span-2 overflow-y-auto custom-scrollbar flex flex-col relative">
                        {selectedBattle ? (
                            <div className="flex flex-col min-h-full">
                                <div className="p-4 bg-slate-900 border-b border-slate-800 flex justify-between items-center shadow-lg sticky top-0 z-20"><div><h2 className="text-xl font-bold text-white flex items-center gap-2">{selectedBattle.name}{selectedBattle.winner === 'us' ? <Trophy className="text-yellow-500" size={20}/> : <Skull className="text-slate-500" size={20}/>}</h2><p className="text-xs text-slate-500 font-mono mt-1">Recorded: {new Date(selectedBattle.date).toUTCString()}</p></div><div className="text-right"><span className={`text-2xl font-black ${selectedBattle.winner === 'us' ? 'text-green-400' : 'text-red-500'}`}>{selectedBattle.winner === 'us' ? 'VICTORY' : 'DEFEAT'}</span></div></div>
                                <div className="p-4 border-b border-white/5 bg-slate-900/30"><VisualTimeline logs={selectedBattle.logs} /></div>
                                <div className="grid grid-cols-2 gap-4 p-4 border-b border-white/5 bg-slate-900/30"><div className="bg-blue-900/10 border border-blue-500/20 rounded-lg p-3 text-center"><span className="text-xs text-blue-400 font-bold uppercase block mb-1">Our Occupation</span><span className="text-2xl font-mono text-white">{formatMinSec(toSeconds(selectedBattle.finalOurTime))}</span></div><div className="bg-red-900/10 border border-red-500/20 rounded-lg p-3 text-center"><span className="text-xs text-red-400 font-bold uppercase block mb-1">Enemy Occupation</span><span className="text-2xl font-mono text-white">{formatMinSec(toSeconds(selectedBattle.finalEnemyTime))}</span></div></div>
                                <div className="flex-1 p-4 pb-12"><h3 className="text-sm font-bold text-slate-400 mb-3 uppercase tracking-wider">Event Timeline</h3><div className="space-y-4 relative pl-4 border-l-2 border-slate-800 ml-2">{selectedBattle.logs.filter(l => ['occupation', 'pet', 'victory'].includes(l.type)).map((log, idx) => ( <div key={idx} className="relative pl-6"><div className={`absolute -left-[29px] top-1 w-4 h-4 rounded-full border-2 ${log.type === 'victory' ? 'bg-yellow-500 border-yellow-300' : log.type === 'occupation' ? 'bg-indigo-500 border-indigo-300' : 'bg-emerald-500 border-emerald-300'}`}></div><div className="flex flex-col"><span className="text-[10px] font-mono text-slate-500 mb-0.5">{log.timestamp.split(' ')[1]} (UTC)</span><span className={`text-sm ${log.type === 'victory' ? 'text-yellow-400 font-bold' : log.type === 'occupation' ? 'text-white' : 'text-emerald-300'}`}>{log.message}</span></div></div> ))}</div></div>
                            </div>
                        ) : ( <div className="flex flex-col items-center justify-center h-full text-slate-600"><Archive size={64} className="mb-4 opacity-20"/><p>Select a battle from the left to view details</p></div> )}
                     </div>
                 </div>
             )}
          </div>
